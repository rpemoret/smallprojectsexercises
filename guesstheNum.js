...
let randomNum = Math.floor(
  Math.random() * (biggestNum - lowestNum) + lowestNum
);
...
while (guessInput !== randomNum) {
  ...
  if (guessInput === randomNum) {
    ...
    guessCounter2--;
    alert(
      "Not quite right yet! Try a lower number \nYou have " +
        guessCounter2 +
        " tries left"
    );
  } else if (guessInput < randomNum) {
    ...
    guessCounter2--;
    alert(
      "Not quite right yet! Try a bigger number \nYou have " +
        guessCounter2 +
        " tries left"
    );
  }
...
if (guessCounter2 === 0) {
  alert("You have used 5 tries. Start a new game or exit");
}
